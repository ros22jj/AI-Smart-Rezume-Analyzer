...
// ══════════════════════════════════════════════════════════════
// atsBuilderRoutes.js — with Redis rate limiting
// POST   /api/ats-builder/build
// GET    /api/ats-builder/history/:clerkUserId
// GET    /api/ats-builder/pdf/:id
// DELETE /api/ats-builder/:id
...
const ATSBuild           = require("../models/ATSBuild");
const { buildATSResume } = require("../services/atsBuilderService");
const { createRateLimiter } = require("../middleware/redisMiddleware");

// ── POST /api/ats-builder/build ───────────────────────────────
// Rate limited: 3 builds per user per minute
router.post("/build", createRateLimiter("ats_build"), async (req, res) => {
  try {
    const {
      clerkUserId,
      userName       = "",
    ...
    await atsBuild.save();
    console.log(`✅ ATS build saved ${atsBuild._id} — +${result.analytics.improvementPercent}pts`);
